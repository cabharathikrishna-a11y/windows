...
object SessionTerminator {
    ...
    suspend fun executeSessionTermination(
        ...
        try {
            ...
            // Action 1: Deduplicate and ensure clean single terminating event in Timeline list
            val lastEvent = currentTimeline.lastOrNull()
            val isLastEndEvent = lastEvent != null && (
            ...
            val updatedTimeline = if (isLastEndEvent && lastEvent != null) {
                // Standardize existing trailing end event to "session_end" without creating a duplicate
                currentTimeline.dropLast(1) + lastEvent.copy(event = "session_end")
            } else {
                val finalEvent = TimelineEvent(deviceId = myDevice, event = "session_end", timestamp = trueTime)
                currentTimeline + finalEvent
            }
            ...
            // Recheck and verify today's focus ms against system database records after session end
            DevicePresenceManager.recheckTodayFocusMsWithSystem(context, email)
